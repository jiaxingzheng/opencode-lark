...
Parses the raw SSE stream from opencode and distributes typed events to whoever needs them. `session-observer.ts` routes TUI-initiated events to Feishu via `FeishuApiClient`.
...
- `TextDelta` events can arrive very rapidly. Never write to Feishu on every delta.
